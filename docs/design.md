...
#### `recall_hint` —— 回想模式专用推导（按需）

回想模式会遮住单词与中文义项。`root_logic` 通常以「→ 中文义项」收尾，
遮完只剩一串方块，提示不足：

```
改前  ▢▢▢（往外压）+ -ion（名词后缀）→ 被压出来的那个东西 → ▢▢▢ / ▢▢▢ / ▢▢▢式
改后  ex-（向外）+ press（压）+ -ion → 被压到外面、变得看得见的那个形式
```

`recall_hint` 写推导路径但不点名中文义项，因此遮罩后依然完整。
质量门 Q12 会在「推导含 3 处以上义项且未提供 recall_hint」时报错。

同一道门还禁止 `core_image` 点名自己的中文义项——那样遮完剩不下画面，
且等于退回中译英，与项目理念相悖。
