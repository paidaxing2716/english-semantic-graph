...
v0.1.1  图谱可用性升级（搜索 + 分层钻取 + 近义词概念中转）← 当前
...
> 详见 [plan-ux-graph.md](plan-ux-graph.md)（已确认方案）

- [ ] 搜索框：单词/词根/中文三路搜索，定位高亮 + 弹详情
- [ ] 图谱分层钻取：默认只显词根+概念，点击展开词族
- [ ] 近义词概念中转：concepts.json 增加 `type: cluster` 聚合概念
- [ ] words.json 增加 `synonym_group` / `synonym_note`
- [ ] 试点 2 组近义词（make a choice / make clear）
- [ ] 详情面板显示「近义词组 + 语境差异」
- [ ] validate.py 校验 synonym_group 引用

**验收**：搜索直达任意词；初始视图 ~20 节点；近义词共享概念零新增连线。
