...
- [x] 第三十七批（12 词，四新词根族）：auctor（作者/权威）
  ...
  - [x] 第三十七至六十七批：**2865 词 · 239 词根**，考研词表覆盖 53.6%
        （2841/5299）。逐批明细不再在此登记——批次已到六十余，登记跟不上，
        反而成了误导来源。当前状态**一律现场重数**，见
        [docs/HANDOFF.md](HANDOFF.md) 开头那段命令。
  - [x] 突破「词根拆解上限 22.8%」：本文件早前估的那个上限，前提是
        「不可拆的词做不了」。用户改了这个前提——日耳曼核心词只要一个
        能记住的画面，不必拆。于是不可拆词从「做不了」变成「换个字段填」，
        覆盖率从 18.5% 一路到 53.6%。
  - [ ] 剩余约 2390 词，拉丁与日耳曼两池，见 HANDOFF「二、剩余工作」

> ⚠️ **本文件的词数会滞后。** 曾有一次照信这里的「694 词 / 下一批是 36」，
> 结果去修了一个早已合并的 build_batch36.py——那 694 是 vetted 清单覆盖数，
> 不是总词数。开工先自己数 `data/words.json`。
>
> ⚠️ 下方「核心词根清单」里的 `press` / `form` / `port` 三个 id **已改名**为
> `premere` / `forma` / `portus`（要给英文单词 press/form/port 腾出 id，
> 见 scripts/migrate_root_ids_latin.py）。清单保留原文作为设计记录，
> 查库请用新 id。
