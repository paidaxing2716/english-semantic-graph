...
for w in miss:
 ...
 origin=('英语词条 pound，现代词义按整体记；与库中 ponere 词族不同源' if w=='pound' else f'英语词条 {w}，现代词义按整体记')
...
for n in (117,118,119):
 part=rows[(n-117)*30:(n-116)*30]
 (ROOT/f'drafts/g_chunk{n}.tsv').write_text('\n'.join('\t'.join(r) for r in part)+'\n',encoding='utf-8',newline='')
print('[BUILD-OK]',len(rows),[len(rows[(n-117)*30:(n-116)*30]) for n in (117,118,119)])
